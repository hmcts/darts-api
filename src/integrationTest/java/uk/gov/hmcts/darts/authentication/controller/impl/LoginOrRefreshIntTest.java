...
class LoginOrRefreshIntTest extends IntegrationBase {
    ...
    private static final String EXPECTED_LOGIN_REDIRECT_URL = "http://localhost:<wiremockPort>/oauth2/v2.0/authorize?client_id=<clientId>&redirect_uri=https%3A%2F%2Fexample.com%2Fhandle-oauth-code&scope=openid&prompt=login&response_mode=form_post&response_type=code";
    ...
    private static final String EXPECTED_LOGIN_REDIRECT_URL_WITH_OVERRIDE = "http://localhost:<wiremockPort>/oauth2/v2.0/authorize?client_id=<clientId>&redirect_uri=https%3A%2F%2Fdarts-portal.com%2Fauth%2Fcallback&scope=openid&prompt=login&response_mode=form_post&response_type=code";
    ...
    void loginOrRefreshShouldReturnRedirectWhenNoAuthHeaderIsSent() throws Exception {
        ...
        mockMvc.perform(requestBuilder)
            ...
            .andExpect(header().string(
                ...
                EXPECTED_LOGIN_REDIRECT_URL.replace("<wiremockPort>", wiremockPort).replace("<clientId>", configurationProperties.getClientId())
            ));
    }
    ...
    void loginOrRefreshShouldReturnRedirectWithOverriddenRedirectUri() throws Exception {
        ...
        mockMvc.perform(requestBuilder)
            ...
            .andExpect(header().string(
                HttpHeaders.LOCATION,
                EXPECTED_LOGIN_REDIRECT_URL_WITH_OVERRIDE.replace("<wiremockPort>", wiremockPort).replace("<clientId>", configurationProperties.getClientId())
            ));
    }
