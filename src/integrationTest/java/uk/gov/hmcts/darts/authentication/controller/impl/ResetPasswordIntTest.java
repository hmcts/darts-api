...
class ResetPasswordIntTest extends IntegrationBase {
    ...
    private static final String EXPECTED_REDIRECT_URL =
        ...
        "client_id=<clientId>&redirect_uri=https%3A%2F%2Fexample.com%2Fhandle-oauth-code&" +
        "scope=openid&prompt=login&response_type=id_token";
    private static final String EXPECTED_REDIRECT_URL_WITH_OVERRIDE =
        "http://localhost:<wiremockPort>/" +
        "B2C_1_darts_externaluser_password_reset/oauth2/v2.0/authorize?" +
        "client_id=<clientId>&redirect_uri=https%3A%2F%2Fdarts-portal.com%2Fauth%2Fcallback&" +
        "scope=openid&prompt=login&response_type=id_token";
    ...
    void resetPasswordShouldReturnRedirect() throws Exception {
        ...
        mockMvc.perform(requestBuilder)
            ...
            .andExpect(header().string(
                ...
                EXPECTED_REDIRECT_URL.replace("<wiremockPort>", wiremockPort).replace("<clientId>", configurationProperties.getClientId())
            ));
    }
    ...
    void resetPasswordShouldReturnRedirectWithOverriddenRedirectUri() throws Exception {
        ...
        mockMvc.perform(requestBuilder)
            ...
            .andExpect(header().string(
                HttpHeaders.LOCATION,
                EXPECTED_REDIRECT_URL_WITH_OVERRIDE.replace("<wiremockPort>", wiremockPort).replace("<clientId>", configurationProperties.getClientId())
            ));
    }
