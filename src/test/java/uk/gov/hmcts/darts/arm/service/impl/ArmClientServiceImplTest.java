...
class ArmClientServiceImplTest {

    private ArmTokenClient armTokenClient;
    private ArmApiClient armApiClient;
    private ArmRpoClient armRpoClient;
    private ArmClientServiceImpl service;

    @BeforeEach
    void setUp() {
        armTokenClient = mock(ArmTokenClient.class);
        armApiClient = mock(ArmApiClient.class);
        armRpoClient = mock(ArmRpoClient.class);
        service = new ArmClientServiceImpl(armTokenClient, armApiClient, armRpoClient);
    }
    ...
    void getToken_returnsResponse() {
        ...
        when(armTokenClient.getToken(request)).thenReturn(response);

        ArmTokenResponse result = service.getToken(request);

        assertEquals(response, result);
        verify(armTokenClient).getToken(request);
    }
    ...
    void getRecordManagementMatter_returnsResponse() {
        ...
        when(armRpoClient.getRecordManagementMatter("Bearer token", request)).thenReturn(response);

        RecordManagementMatterResponse result = service.getRecordManagementMatter("Bearer token", request);

        assertEquals(response, result);
        verify(armRpoClient).getRecordManagementMatter("Bearer token", request);
    }
    ...
    void getStorageAccounts_returnsResponse() {
        ...
        when(armRpoClient.getStorageAccounts("Bearer token", request)).thenReturn(response);

        StorageAccountResponse result = service.getStorageAccounts("Bearer token", request);

        assertEquals(response, result);
        verify(armRpoClient).getStorageAccounts("Bearer token", request);
    }
    ...
    void createExportBasedOnSearchResultsTable_returnsResponse() {
        ...
        when(armRpoClient.createExportBasedOnSearchResultsTable("Bearer token", request)).thenReturn(response);

        CreateExportBasedOnSearchResultsTableResponse result = service.createExportBasedOnSearchResultsTable("Bearer token", request);

        assertEquals(response, result);
        verify(armRpoClient).createExportBasedOnSearchResultsTable("Bearer token", request);
    }
    ...
    void removeProduction_returnsResponse() {
        ...
        when(armRpoClient.removeProduction("Bearer token", request)).thenReturn(response);

        RemoveProductionResponse result = service.removeProduction("Bearer token", request);

        assertEquals(response, result);
        verify(armRpoClient).removeProduction("Bearer token", request);
    }
    ...
    void getMasterIndexFieldByRecordClassSchema_returnsResponse() {
        ...
        when(armRpoClient.getMasterIndexFieldByRecordClassSchema("Bearer token", request)).thenReturn(response);

        service.getMasterIndexFieldByRecordClassSchema("Bearer token", request);

        verify(armRpoClient).getMasterIndexFieldByRecordClassSchema("Bearer token", request);
    }
    ...
    void getProfileEntitlementResponse_returnsResponse() {
        ...
        when(armRpoClient.getProfileEntitlementResponse("Bearer token", request)).thenReturn(response);

        service.getProfileEntitlementResponse("Bearer token", request);

        verify(armRpoClient).getProfileEntitlementResponse("Bearer token", request);
    }

    @Test
    void addAsyncSearch_returnsResponse() {
        ArmAsyncSearchResponse armAsyncSearchResponse = new ArmAsyncSearchResponse();
        when(armRpoClient.addAsyncSearch("Bearer token", "body")).thenReturn(armAsyncSearchResponse);

        service.addAsyncSearch("Bearer token", "body");

        verify(armRpoClient).addAsyncSearch("Bearer token", "body");
    }
    ...
    void getIndexesByMatterId_returnsResponse() {
        ...
        when(armRpoClient.getIndexesByMatterId("Bearer token", request)).thenReturn(response);

        service.getIndexesByMatterId("Bearer token", request);

        verify(armRpoClient).getIndexesByMatterId("Bearer token", request);
    }
    ...
    void saveBackgroundSearch_returnsResponse() {
        ...
        when(armRpoClient.saveBackgroundSearch("Bearer token", request)).thenReturn(response);

        service.saveBackgroundSearch("Bearer token", request);

        verify(armRpoClient).saveBackgroundSearch("Bearer token", request);
    }
    ...
    void getExtendedSearchesByMatter_returnsResponse() {
        ...
        when(armRpoClient.getExtendedSearchesByMatter("Bearer token", "body")).thenReturn(response);

        service.getExtendedSearchesByMatter("Bearer token", "body");

        verify(armRpoClient).getExtendedSearchesByMatter("Bearer token", "body");
    ...
    void getProductionOutputFiles_returnsResponse() {
        ...
        when(armRpoClient.getProductionOutputFiles("Bearer token", request)).thenReturn(response);
    ...
    void downloadProductionStub_returnsResponse() {
        ...
        verify(armRpoClient).downloadProduction("Bearer token", "eodIds", "productionExportFileId");
    }
    ...
    void availableEntitlementProfiles_returnsResponse() {
        ...
        verify(armTokenClient).availableEntitlementProfiles("Bearer token", EmptyRpoRequest.builder().build());
    }
    ...
    void selectEntitlementProfile_returnsResponse() {
        ...
        verify(armTokenClient).selectEntitlementProfile("Bearer token", "profileId", request);
    }
