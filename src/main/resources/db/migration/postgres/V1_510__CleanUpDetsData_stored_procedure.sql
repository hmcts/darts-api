DROP FUNCTION IF EXISTS darts.dets_cleanup_eod_osr_rows;
DROP PROCEDURE IF EXISTS darts.dets_cleanup_eod_osr;
DROP TYPE IF EXISTS darts.id_location_pair;

CREATE TYPE darts.id_location_pair AS
(
    osr_uuid      BIGINT,
    dets_location TEXT
);
...
CREATE OR REPLACE PROCEDURE darts.dets_cleanup_eod_osr(
    IN pi_limit INTEGER,
    IN pi_last_modified_ts TIMESTAMPTZ,
    OUT po_results_array darts.id_location_pair[]
)
    LANGUAGE 'plpgsql'
AS
$$
DECLARE
    ...
    v_incomplete_cleanup       darts.id_location_pair[];
...
BEGIN
    ...
    BEGIN
        ...
        SELECT array_agg(eod_id)
             ...
             , array_agg((osr_uuid, dets_location)::darts.id_location_pair)
        INTO v_dets_eod_ids
            , v_arm_eod_ids
            , po_results_array
        FROM (SELECT osr.osr_uuid, osr.dets_location, eod_dets.eod_id, eod_arm.eod_id AS arm_eod_id
              FROM darts.object_state_record osr
                       JOIN darts.external_object_directory eod_dets
        ...
        SELECT array_agg((osr_uuid, dets_location)::darts.id_location_pair ORDER BY osr_uuid) AS id_location_pairs
...
CREATE OR REPLACE FUNCTION darts.dets_cleanup_eod_osr_rows(pi_limit int, pi_last_modified_ts timestamptz)
    RETURNS TABLE
            (
                osr_uuid      bigint,
                dets_location text
...
$$
DECLARE
    v_arr darts.id_location_pair[];
BEGIN
    CALL darts.dets_cleanup_eod_osr(pi_limit, pi_last_modified_ts, v_arr);
    RETURN QUERY SELECT (x).osr_uuid, (x).dets_location
                 FROM unnest(COALESCE(v_arr, '{}'::darts.id_location_pair[])) x;
END
$$;