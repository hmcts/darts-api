...
ALTER TABLE wk_case_activity_data DROP COLUMN rownum;
ALTER TABLE wk_case_activity_data ALTER COLUMN closed_date_type CHARACTER VARYING(100);
...
CREATE UNIQUE INDEX wk_case_confidence_level_pk ON wk_case_confidence_level(cas_id);
ALTER TABLE wk_case_confidence_level ADD PRIMARY KEY USING INDEX  wk_case_confidence_level_pk;

-- WK_CASE_CORRECTION
ALTER TABLE wk_case_correction ADD COLUMN cmr_eve_id BIGINT;
...
CREATE UNIQUE INDEX wk_crah_is_current_by_creation_pk ON wk_crah_is_current_by_creation(cas_id);
...
CREATE UNIQUE INDEX wk_crah_is_current_by_logic_pk ON wk_crah_is_current_by_logic(cas_id);
...
CREATE UNIQUE INDEX wk_rr_is_current_by_creation_pk ON wk_rr_is_current_by_creation(cas_id);
...
CREATE UNIQUE INDEX wk_rr_is_current_by_logic_pk ON wk_rr_is_current_by_logic(cas_id);
