...
UPDATE user_account user_account SET created_by = 0 WHERE created_by IS NULL OR NOT EXISTS (SELECT 1 FROM user_account ua WHERE ua.usr_id = user_account.created_by);
...
UPDATE user_account user_account SET last_modified_by = 0 WHERE last_modified_by IS NULL OR NOT EXISTS (SELECT 1 FROM user_account ua WHERE ua.usr_id = user_account.last_modified_by);
