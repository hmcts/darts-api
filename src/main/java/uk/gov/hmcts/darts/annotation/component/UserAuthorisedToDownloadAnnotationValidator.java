...
import static uk.gov.hmcts.darts.annotation.errors.AnnotationApiError.USER_NOT_AUTHORISED_TO_DOWNLOAD;
import static uk.gov.hmcts.darts.common.enums.SecurityRoleEnum.ADMIN;

@RequiredArgsConstructor
@Component
public class UserAuthorisedToDownloadAnnotationValidator implements Validator<Integer> {
    ...
    public void validate(Integer id) {
        ...
        if (!authorisationApi.userHasOneOfRoles(List.of(ADMIN)) && !annotation.isOwnedBy(currentUser)) {
            throw new DartsApiException(USER_NOT_AUTHORISED_TO_DOWNLOAD);
        }
    }
}
