package uk.gov.hmcts.darts.retentions.validation;
        ...
        if (BooleanUtils.isTrue(postRetentionRequest.getIsPermanentRetention())) {
            if (postRetentionRequest.getRetentionDate() != null) {
                throw new DartsApiException(
                    RetentionsApiError.INVALID_REQUEST,
                    "Both 'is_permanent_retention' and 'retention_date' cannot be set, must be either one or the other."
                );
            }
        } else {
            if (postRetentionRequest.getRetentionDate() == null) {
                throw new DartsApiException(RetentionsApiError.INVALID_REQUEST, "Either 'is_permanent_retention' or 'retention_date' must be se.");
            }
        }
    }
}
