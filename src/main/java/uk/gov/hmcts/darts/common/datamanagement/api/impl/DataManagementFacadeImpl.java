...
public class DataManagementFacadeImpl implements DataManagementFacade {
                ...
                if (container.isPresent()) {
                    ...
                    if (containerName.isPresent() && processObjectDirectoryForContainerType(dataToDownload, type, isFetchfromDets)) {
                        ...
                        if (!dataToDownload.getDownloadReport().isSuccessfullyDownloaded()) {
                            ...
                            if (!processReport(dataToDownload, success, processed, type)) {
                                return;
                            }
                        }
                    }
        ...
        if (!dataToDownload.getDownloadReport().isSuccessfullyDownloaded()) {
            if (dataToDownload.getDirectory().isForLocationType(ExternalLocationTypeEnum.UNSTRUCTURED)) {
                ...
                try {
                    ...
                    dataToDownload.getDownloadReport().markInputStream(stream);
                    ...
                    processReport(dataToDownload, true, processed, DatastoreContainerType.ARM);
                } catch (Exception e) {
                    log.error("Download failed from arm fallback. Giving up", e);
                    processReport(dataToDownload, false, processed, DatastoreContainerType.ARM);
                }
            }
        }
    }
    ...
    private boolean processReport(DownloadableExternalObjectDirectory download,
        ...
        download.getDownloadReport().markProcessed();
        if (downloadSuccess) {
            download.getDownloadReport().markSuccess(processContainer);
        }
