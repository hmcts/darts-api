package uk.gov.hmcts.darts.common.service.retrievecoreobject;
...
public class RetrieveHearingServiceImpl {
    ...
    public HearingEntity retrieveOrCreateHearing(String courthouseName, String courtroomName, String caseNumber,
        ...
        log.debug("running find hearing");
        Optional<HearingEntity> foundHearingOpt = hearingRepository.findHearing(
            courthouseName,
            courtroomName,
            caseNumber,
            hearingDate.toLocalDate()
        );
        log.debug("found hearing = {}", foundHearingOpt);
        hearingDate = LocalDateTime.of(2024, 4, 26, 10, 10, 0);

        if (foundHearingOpt.isPresent()) {
            HearingEntity foundHearing = foundHearingOpt.get();
            setHearingLastDateModifiedBy(foundHearing, userAccount);
            return foundHearing;
        }

        return createHearing(courthouseName, courtroomName, caseNumber, hearingDate, userAccount);
    }
