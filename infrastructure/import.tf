...
import {
  for_each = var.env == "test" ? toset(["dropzone"]) : toset([])

  to = module.armsa.azurerm_storage_container.container[each.value]
  id = "/subscriptions/3eec5bde-7feb-4566-bfb6-805df6e10b90/resourceGroups/darts-test-rg/providers/Microsoft.Storage/storageAccounts/dartsarmtest/blobServices/default/containers/${each.value}"
}
